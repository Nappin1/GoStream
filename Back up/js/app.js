...
function renderContinueWatching() {
    ...
    appState.continueWatching.slice(0, 20).forEach(item => {
        ...
        if (item.type === 'tv' && item.season) {
            badge = `<div class="absolute top-2 right-2 bg-primary text-white text-[10px] font-bold px-2 py-1 rounded shadow-md z-10">S${item.season} E${item.episode}</div>`;
        } else if (item.type === 'anime') {
            badge = `<div class="absolute top-2 right-2 bg-primary text-white text-[10px] font-bold px-2 py-1 rounded shadow-md z-10">EP ${item.episode}</div>`;
        }
...
function playAnilistEp(ep) {
    ...
    Array.from(wrappers).forEach(wrapper => {
        ...
        if (btnEpNum === ep) {
            // ACTIVE
            b.classList.remove('bg-input-bg', 'text-white', 'hover:border-primary', 'hover:text-primary');
            b.classList.add('bg-primary', 'text-white', 'font-bold', 'border-primary');
        } else {
            // INACTIVE
            b.classList.add('bg-input-bg', 'text-white', 'hover:border-primary', 'hover:text-primary');
            b.classList.remove('bg-primary', 'text-white', 'font-bold', 'border-primary');
        }
    });
    ...
    try {
        ...
        container.innerHTML = `
                <div class="container mx-auto px-6 sm:px-8 lg:px-12 md:pt-20 md:pt-32 pb-12">
                    ...
                    <div id="tmdb-player-row" class="flex flex-col lg:flex-row lg:items-start gap-6">
                        <!-- Player Container -->
                        <div id="tmdb-player-col" class="${type === 'tv' || collectionData ? 'w-full lg:w-3/4' : 'w-full'} rounded overflow-hidden border border-border-color bg-[#020617] shadow-xl mb-8 lg:mb-0">
                        ...
                        <div id="tmdb-sidebar-col" class="${type === 'tv' || collectionData ? 'w-full lg:w-1/4 bg-card-bg border border-border-color rounded flex flex-col overflow-hidden' : 'hidden'}">
                            ${type === 'tv' ? `
                            <div class="p-3 border-b border-border-color px-4 bg-[#111] flex justify-between items-center flex-shrink-0">
                                 ...
                                 <button id="ep-view-btn" onclick="toggleEpisodeView()" class="w-10 h-8 flex items-center justify-center rounded backdrop-blur-md bg-white/5 border border-white/10 text-white hover:text-white hover:bg-primary hover:border-primary transition-all" title="Toggle Layout">
                                     ${appState.episodeViewMode === 'thumbnails' ? '<i class="ph ph-grid-nine"></i>' : '<i class="ph ph-list"></i>'}
                                 </button>
                            </div>
                            <div id="tv-episodes-grid" class="grid grid-cols-4 sm:grid-cols-6 lg:grid-cols-3 gap-2 p-3 overflow-y-auto custom-scroll flex-1 content-start max-h-96 lg:max-h-[var(--sidebar-list-h,400px)]"></div>
                            
                            <div id="tv-seasons-container" class="flex-shrink-0 border-t border-border-color hidden flex-col relative w-full">
                                ...
                                <div id="tv-seasons-list" class="horizontal-scroll-container p-2 custom-scroll bg-[#020617]/10 items-start"></div>
                            </div>
                            ` : ''}

                            <div id="tmdb-collection-container" class="${collectionData ? 'flex flex-col flex-1' : 'hidden'}">
                                <div class="p-3 border-b border-border-color bg-[#111] flex justify-between items-center flex-shrink-0">
                                    <h4 class="text-base font-bold text-text-main mb-0">${collectionData ? collectionData.name : 'Collection'}</h4>
                                </div>
                                <div id="tmdb-collection-list" class="grid grid-cols-2 gap-3 p-3 overflow-y-auto custom-scroll items-start auto-rows-max max-h-96 lg:max-h-[var(--sidebar-list-h,400px)]"></div>
                            </div>
                        </div>
                    </div>
        ...
        // Sync sidebar height to player column height on desktop
        const syncSidebarHeight = () => {
            const playerCol = document.getElementById('tmdb-player-col');
            const sidebarCol = document.getElementById('tmdb-sidebar-col');
            if (!playerCol || !sidebarCol) return;
            if (window.innerWidth >= 1024) {
                const playerH = playerCol.getBoundingClientRect().height;
                // Header row inside sidebar is ~48px; subtract it so the list fills the rest
                const headerH = sidebarCol.querySelector('.flex-shrink-0') ? sidebarCol.querySelector('.flex-shrink-0').getBoundingClientRect().height : 48;
                const listH = Math.max(200, playerH - headerH);
                sidebarCol.style.setProperty('--sidebar-list-h', listH + 'px');
                sidebarCol.style.height = playerH + 'px';
            } else {
                sidebarCol.style.height = '';
                sidebarCol.style.removeProperty('--sidebar-list-h');
            }
        };
